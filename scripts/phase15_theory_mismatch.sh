...
GROUP_CORRELATION_SAMPLE="${GROUP_CORRELATION_SAMPLE:-200000}"
GROUP_BOOTSTRAP_REPS="${GROUP_BOOTSTRAP_REPS:-20}"
GROUP_BOOTSTRAP_SAMPLE_SIZE="${GROUP_BOOTSTRAP_SAMPLE_SIZE:-20000}"
...
python -m casafety.theory_mismatch \
  ...
  --group-correlation-sample "$GROUP_CORRELATION_SAMPLE" \
  --group-bootstrap-reps "$GROUP_BOOTSTRAP_REPS" \
  --group-bootstrap-sample-size "$GROUP_BOOTSTRAP_SAMPLE_SIZE" \
