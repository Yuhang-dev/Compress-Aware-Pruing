...
# It installs experiment dependencies with the preserved pip cache and downloads
# model assets into the preserved Hugging Face cache.
...
python -m pip install \
  --index-url https://pypi.org/simple \
  --cache-dir "$PIP_CACHE_DIR" \
  vllm lm-eval bitsandbytes auto-gptq autoawq
...
echo "Phase 0 package/model download completed."
