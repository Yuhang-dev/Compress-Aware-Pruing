...
MODEL_TAG="${MODEL_TAG:-qwen25_3b_instruct}"
OUTPUT_DIR="${OUTPUT_DIR:-results/phase2_qwen3b_sparsegpt_scan}"
SHARD_ROOT="${SHARD_ROOT:-${OUTPUT_DIR}_shards}"
LOG_DIR="${LOG_DIR:-logs}"

SPARSITIES="${SPARSITIES:-0.45 0.50 0.55}"
PRUNERS="${PRUNERS:-sparsegpt}"
...
# SparseGPT integration knobs. These are intentionally env-controlled so the
# remote run can trade fidelity and wall clock without changing code.
export SPARSEGPT_BLOCKSIZE="${SPARSEGPT_BLOCKSIZE:-128}"
export SPARSEGPT_DAMP="${SPARSEGPT_DAMP:-0.01}"
export SPARSEGPT_HESSIAN_BLOCK="${SPARSEGPT_HESSIAN_BLOCK:-2048}"
export SPARSEGPT_MAX_EXACT_IN_FEATURES="${SPARSEGPT_MAX_EXACT_IN_FEATURES:-4096}"
export SPARSEGPT_MAX_SAMPLES="${SPARSEGPT_MAX_SAMPLES:-512}"
export SPARSEGPT_SEED="${SPARSEGPT_SEED:-$SEED}"
...
run_dense() {
  ...
  mkdir -p "$shard_dir"
  echo "[qwen3b-sparsegpt-scan] dense baseline"
...
run_cell() {
  ...
  local log_file="$LOG_DIR/qwen3b_sparsegpt_scan_${label}.log"
  mkdir -p "$shard_dir"
  echo "[qwen3b-sparsegpt-scan] launching pruner=$pruner sparsity=$sparsity log=$log_file"
  (
    set -euo pipefail
    python -m casafety.phase0_smoke_eval \
      --config configs/base.yaml \
...
run_dense >"$LOG_DIR/qwen3b_sparsegpt_scan_dense.log" 2>&1
...
decision = {
    ...
    "dense_ppl": dense_ppl,
    "sparsegpt": {
        "blocksize": int("$SPARSEGPT_BLOCKSIZE"),
        "damp": float("$SPARSEGPT_DAMP"),
        "hessian_block": int("$SPARSEGPT_HESSIAN_BLOCK"),
        "max_exact_in_features": int("$SPARSEGPT_MAX_EXACT_IN_FEATURES"),
        "max_samples": int("$SPARSEGPT_MAX_SAMPLES"),
    },
...
print(f"[qwen3b-sparsegpt-scan] wrote {output_dir / 'joined.csv'}")
print(f"[qwen3b-sparsegpt-scan] wrote {output_dir / 'decision.json'}")
PY
